...
class EditEmailVerifyCodeCotrollerImp extends EditEmailVerifyCodeCotroller {
  ...
  verfyCodeAndGoToSuccessView({required verificationCode}) async {
    if (vrifyCode != null || vrifyCode!.length != 5) {
      ...
      update();
      var response = await profileData.editEmailVerifyCode(
        userEmail: userEmail,
        newEmail: newEmail,
  ...
  @override
  resend() {
    profileData.editEmailReSendCode(
      userEmail: userEmail,
      newEmail: userEmail,
    );
