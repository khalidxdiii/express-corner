...
class EditEmailControllerImp extends EditEmailController {
  ...
  sendVerificationCode() async {
    if (key.currentState!.validate()) {
      ...
      update();
      var response = await profileData.editEmail(
      ...
      if (StatusRequest.success == statusRequest) {
        if (response['status'] == "success") {
          Get.toNamed(AppRoute.profileEditEmailVerifyCode, arguments: {
            "userEmail": myServices.sharedPreferences.getString("email")!,
            "newEmail": newEmail.text.trim(),
          });
          Get.snackbar(
